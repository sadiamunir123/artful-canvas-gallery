...
const LOVABLE_PREVIEW_ORIGIN = "https://id-preview--271daa9e-a0d1-4284-8864-d8601052c602.lovable.app";
const OAUTH_MESSAGE_ORIGINS = [LOVABLE_OAUTH_ORIGIN, "https://lovable.dev"];
...
const signInWithLocalGooglePopup = () =>
  new Promise<{ access_token: string; refresh_token: string }>((resolve, reject) => {
    ...
    const popup = window.open(
      `${LOVABLE_OAUTH_ORIGIN}/initiate?${params.toString()}`,
      "oauth",
    ...
    function onMessage(event: MessageEvent) {
      ...
      if (response.state !== state) {
        finishWithError("Google sign-in state did not match. Please try again.");
        return;
      }
      ...
      if (!response.access_token || !response.refresh_token) {
        finishWithError("No admin session was received from Google sign-in.");
        return;
      }
    ...
    const closedCheck = window.setInterval(() => {
      if (popup.closed) finishWithError("Google sign-in was cancelled.");
    }, 500);
